...
@pytest.fixture
def pre_name_b_plier_res(data_dir: Traversable, test_plier_res: PLIERResults) -> PLIERResults:
...
@pytest.fixture
def prior_names(common_dir: Path) -> list[str]:
    return pd.read_csv(common_dir.joinpath("plier_res_b.csv.gz"), index_col=0).index.to_list()


def test_name_b(test_plier_res: PLIERResults, prior_names: list[str]) -> None:
