...
@given(
    st.lists(st.floats(min_value=-1e9, max_value=1e9, allow_nan=False, allow_infinity=False), min_size=20),
    st.lists(st.floats(min_value=-1e9, max_value=1e9, allow_nan=False, allow_infinity=False), min_size=20),
)
...
@pytest.fixture
def test_pos():
    posval_file = ir.files("tests").joinpath("data", "mannwhitney_conf_int", "posval.csv.gz")
...
@pytest.fixture
def test_neg():
    negval_file = ir.files("tests").joinpath("data", "mannwhitney_conf_int", "negval.csv.gz")
...
def test_mannwhitneyu_conf_int_two_sided(test_pos, test_neg):
    ...
    assert (low, high) == (pytest.approx(0.0023365916996431397), pytest.approx(0.05108464103143653))
