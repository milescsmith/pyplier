...
def PLIER(
  ...
  pathwaySelection: str="complete"
    ...
    Args:
        data ([type]): the data to be processed with genes in rows and samples in columns. Should be z-scored or set scale=T
        priorMat ([type]): the binary prior information matrix with genes in rows and pathways/genesets in columns
        svdres ([type], optional): Pre-computed result of the svd decomposition for data. Defaults to None.
        num_LVs ([type], optional): The number of latent variables to return, leave as None to be set automatically using the num_pc 'elbow' method. Defaults to None.
        L1 ([type], optional): L1 constant, leave as None to automatically select a value.
        L2 ([type], optional): L2 constant, leave as None to automatically select a value
        L3 ([type], optional): L3 constant, leave as None to automatically select a value. Sparsity in U should be instead controlled by setting frac
        frac (float, optional): The fraction of LVs that should have at least 1 prior inforamtion association, used to automatically set L3. Defaults to 0.7.
        max_iter (int, optional): Maximum number of iterations to perform. Defaults to 350
        trace ([type], optional): Display progress information. Defaults to F.
        scale ([type], optional): Z-score the data before processing. Defaults to T.
        Chat ([type], optional): A ridge inverse of priorMat, used to select active pathways, expensive to compute so can be precomputed when running PLIER multiple times. Defaults to None.
        maxPath (int, optional): The maximum number of active pathways per latent variable. Defaults to 10.
        doCrossval ([type], optional): Whether or not to do real cross-validation with held-out pathway genes. Alternatively, all gene annotations are used and only pseudo-crossvalidation is done. The latter option may be preferable if some pathways of interest have few genes. Defaults to T.
        penalty_factor ([type], optional): A vector equal to the number of columns in priorMat. Sets relative penalties for different pathway/geneset subsets. Lower penalties will make a pathway more likely to be used. Only the relative values matter. Internally rescaled. Defaults to None
        glm_alpha (float, optional): Set the alpha for elastic-net. Defaults to 0.9.
        minGenes (int, optional): The minimum number of genes a pathway must have to be considered. Defaults to 10.
        tol ([type], optional): Convergence threshold. Defaults to 1e-06.
        seed (int, optional): Set the seed for pathway cross-validation. Defaults to 123456.
        allGenes ([type], optional): Use all genes. By default only genes in the priorMat matrix are used. Defaults to F.
        rseed ([type], optional): Set this option to use a random initialization, instead of SVD. Defaults to None.
        pathwaySelection (str, optional): Pathways to be optimized with elstic-net penalty are preselected based on ridge regression results. 'Complete' uses all top pathways to fit individual LVs. 'Fast' uses only the top pathways for the single LV in question. Defaults to "complete".
